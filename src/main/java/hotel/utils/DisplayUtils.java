...
public class DisplayUtils
{
    ...
    public static void printReservations(List<Reservation> reservations)
    {
        ...
        for(int i=0;i<reservations.size();i++)
        {
            ...
            System.out.printf("%-18s %-10s %-15s %-15s %-10.2f %s%n",res.getReservationId(), res.getRoomId(), res.getCheckInDate(), res.getCheckOutDate(), res.getTotalCost(),
                    res.getStatus());
        }
    ...
    public static void printGuests(List<Guest> guests)
    {
        ...
        for(int i=0;i<guests.size();i++)
        {
            ...
            System.out.printf("%-12s %-15s %-10s %.2f%n",g.getGuestId(), g.getUsername(), g.getGender(), g.getBalance());
        }
